    ...
    if (structOffset === -1) {
      ...
      if (hasChildDirs) {
        if (
          ...
          console.log(
            "[error] Has child dirs but `payload_recursive_size` same as `payload_size`:",
            dir
          );
        }
      } else {
        if (
          ...
          console.log(
            "[warn] Doesn't have child dirs but `payload_recursive_size` isn't same as `payload_size`:",
            dir
          );
        }
      }
